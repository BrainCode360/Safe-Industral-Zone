...
class Processor():
    def __init__(self, model):
        ...
        self.output_queue_L = Queue(maxsize=1)
        self.output_queue_R = Queue(maxsize=1)
        self.output_queue_F = Queue(maxsize=1)
        self.output_queue_B = Queue(maxsize=1)
        self.det = None
        
        with open('back.npy', 'rb') as f:
           self.back_polygone_pts = np.load(f)
        f.close()

        with open('left.npy', 'rb') as f:
           self.left_polygone_pts = np.load(f)
        f.close()
        
        with open('right.npy', 'rb') as f:
           self.right_polygone_pts = np.load(f)
        f.close()
        
        with open('front.npy', 'rb') as f:
           self.front_polygone_pts = np.load(f)
        # f.close()
        ...
        # self.left_polygone_pts = np.array([[319, 317], [299, 237], [244, 162], [186, 111], [129, 82], [66, 69], [4, 67], [0, 317]], np.int32)
        # self.right_polygone_pts = np.array([[260, 318], [309, 89], [207, 61], [133, 62], [73, 94], [29, 148], [4, 226], [1, 317]], np.int32)
    ...
    def detect(self, img, frame_Type):
        ...
        output_frame = self.post_process(self.frame_queue)
        self.frame_Type = None
    ...
    def post_process(self, frame_queue):
        """
                ...
                if (outputs is not None):
                    ...
                    if(frame_Type == 'F'):
                        ...
                        if(not (det == [])):
                            ...
                            img = draw_redZone_Back(img, self.left_polygone_pts, pt1=(176, 160), pt2=(799, 598))
                            output_Frame = cvDrawBoxes(det, img, self.front_polygone_pts)
                        ...
                        if(not (det == [])):
                            img = draw_redZone_Back(img, self.right_polygone_pts, pt1 = (176, 160), pt2=(799, 598))
                            output_Frame = cvDrawBoxes(det, img, self.front_polygone_pts)
